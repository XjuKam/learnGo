...
func main() {
	...
	for {
		...
		v3 = text[opIndex+1:]
		fmt.Println(text, opIndex, v1, v2, v3)
		...
		fmt.Println(text, opIndex, v1, v2, v3, v1int, v3int)

		// Проверяем систему нумерации
		if OnlyArabRome(v1int, v3int) == "rome" {
			key, found := GetKeyByValueRome(mm, v1)
			valueMm := mm[key]
			fmt.Println(text, opIndex, v1, v2, v3, v1int, v3int, found)
			if found {
				v1int = valueMm.arabNum
				fmt.Println(text, opIndex, v1, v2, v3, v1int, v3int, found, valueMm, valueMm.arabNum)
			} else {
			...
			fmt.Println(text, opIndex, v1, v2, v3, v1int, v3int, found, valueMm, valueMm.arabNum, romeOper)
			// Выполнение операции
			...
			fmt.Println(text, opIndex, v1, v2, v3, v1int, v3int, found, valueMm, valueMm.arabNum, romeOper, result)
